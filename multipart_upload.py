...
def main(transfer_file, bucket_name, s3_key_name=None, use_rr=True,
         make_public=True, AWS_USER_ID='', AWS_SECRET_KEY=''):
    if s3_key_name is None:
        s3_key_name = os.path.basename(transfer_file)
#    calling_format = boto.s3.connection.OrdinaryCallingFormat() if '.' in bucket_name else  'boto.s3.connection.SubdomainCallingFormat'
    calling_format =  'boto.s3.connection.SubdomainCallingFormat'
    conn = boto.connect_s3(AWS_USER_ID, AWS_SECRET_KEY, calling_format=calling_format)
    bucket = conn.get_bucket(bucket_name, validate=False)#.lookup(bucket_name)
